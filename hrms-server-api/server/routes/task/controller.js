...
const addTaskById = ((req, res) => {
    ...
    Task.findOne({ userId: req.params.id, date: getCurrentDate() })
        .then((result) => {
            Task.findByIdAndUpdate(result._id, { $push: { task: { title: req.body.title, description: req.body.description, date: Date.now(), status: req.body.status } } })
                .then((resul) => {

                    Task.findOne({ "_id": resul._id }).populate("userId")
                        .then((result) => {
...
const showTasks = ((req, res) => {
    Task.find().populate("userId").lean(true)
        .then((result) => {
            res.json({ response: true, result: result });
        })
...
const showTaskByIdAndDate = ((req, res) => {
    ...
    Task.findOne({ userId: req.params.id, date: new Date(req.params.date) }).populate("userId")
        .then((result) => {
            res.json({ response: true, result: result });
        })
        .catch(err => console.log("error in addUserDetail", err));
